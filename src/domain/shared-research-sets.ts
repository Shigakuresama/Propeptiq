...
export function createSharedResearchSet(input: unknown): Result<SharedResearchSet, SharedSetError> {
  ...
  for (let index = 0; index < input.items.length; index += 1) {
    const item = input.items[index]; if (!isRecord(item) || extras(item, ["productId", "quantity", "active"]) !== null || typeof item.productId !== "string" || item.productId.trim().length === 0 || ids.has(item.productId)) return fail("invalid_item", `items[${index}].productId`);
...
export function projectPublicSharedResearchSet(input: unknown): Result<Readonly<{ code: string; label: string; omittedProductIds: readonly string[]; items: readonly Readonly<{ productId: string; quantity: number; name: string }>[] }>, SharedSetError> {
  ...
  if (!isRecord(input.sharedSet) || !dense(input.sharedSet.items) || typeof input.sharedSet.code !== "string" || typeof input.sharedSet.label !== "string") return fail("invalid_input", "sharedSet");
  ...
  for (let index = 0; index < input.currentProducts.length; index += 1) { const product = input.currentProducts[index]; if (!isRecord(product) || extras(product, ["id", "active", "name"]) !== null || typeof product.id !== "string" || product.id.trim().length === 0 || typeof product.active !== "boolean" || typeof product.name !== "string" || product.name.trim().length === 0 || products.has(product.id)) return fail("invalid_input", `currentProducts[${index}]`); products.set(product.id, { active: product.active, name: product.name }); }
  const items: Array<{ productId: string; quantity: number; name: string }> = []; const omittedProductIds: string[] = [];
  for (let index = 0; index < input.sharedSet.items.length; index += 1) { const item = input.sharedSet.items[index]; if (!isRecord(item) || extras(item, ["productId", "quantity"]) !== null || typeof item.productId !== "string") return fail("invalid_input", `sharedSet.items[${index}]`); const quantity = item.quantity; if (!safeInteger(quantity) || quantity < 1 || quantity > 25) return fail("invalid_input", `sharedSet.items[${index}]`); const product = products.get(item.productId); if (product?.active) items.push({ productId: item.productId, quantity, name: product.name }); else omittedProductIds.push(item.productId); }
  return Object.freeze({ ok: true, value: freeze({ code: input.sharedSet.code, label: input.sharedSet.label, omittedProductIds, items }) });
}
