...
export function ProductPurchasePanel({ product, pricing }: ProductPurchasePanelProps) {
  ...
  const quantityIsValid = /^(?:[1-9]|1[0-9]|2[0-5])$/u.test(quantityDraft);
  const quantity = quantityIsValid ? Number(quantityDraft) : lastValidQuantity;
  ...
  return <section className="mt-10 space-y-8" aria-labelledby="purchase-heading">
    ...
    <div><h3 className="mb-3 font-heading text-2xl text-ink">Quantity</h3><QuantityTierSelector quantity={lastValidQuantity} quantityDraft={quantityDraft} errorId="quantity-error" errorMessage={errorMessage} onQuantityDraftChange={(draft) => { setQuantityDraft(draft); if (/^(?:[1-9]|1[0-9]|2[0-5])$/u.test(draft)) setLastValidQuantity(Number(draft)); }} onQuantitySelect={chooseQuantity} /></div>
