        ...
        return <button key={option.quantity} type="button"
          ...
          <span className="bundle-option__bottles" aria-hidden="true">
            {Array.from({ length: bulk ? 3 : Math.min(option.bottleCount, 4) }, (_, index) => <Image
              ...
              src={photograph?.src ?? scene.src} sizes="64px" className="bundle-option__bottle" />)}
            {bulk ? <span className="bundle-option__count">×{option.bottleCount}</span> : null}
          </span>
          <span className="bundle-option__quantity">{option.bottleCount} bottles</span>
          {variant.packageQuantity > 1 ? <span className="text-xs text-muted-ink">{option.quantity} unit{option.quantity === 1 ? "" : "s"}</span> : null}
          <strong className="bundle-option__total tabular-nums">{formatStorefrontMoney(option.price.lineSubtotalMinor)} <span>total</span></strong>
          <span className="bundle-option__discount">Save {option.discountBps / 100}% extra</span>
        </button>;
      })}
    </div>
