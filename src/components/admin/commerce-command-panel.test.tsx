...
function resource(slug: "orders" | "refunds" | "shipments") {
  const selected = resourceBySlug(slug);
  if (!selected) throw new Error(`missing resource ${slug}`);
  return selected;
}

function snapshot<Resource extends "orders" | "refunds" | "shipments">(
  value: object,
  ...
  it("keeps preparation separate from UUID-only handoff, delivery, and exception commands", () => {
    const pending = snapshot<"shipments">({
      ...
      items: [{
        ...
        releaseExpiresAt: null, carrier: "REDACTED", trackingReference: "REDACTED",
        state: "pending", handedOffAt: null, deliveredAt: null,
        createdAt: "2026-08-26T12:00:00.000Z", updatedAt: "2026-08-26T12:00:00.000Z",
        handoffConfirmationBoundary: "task6_managed",
      }],
    });
    const { rerender } = render(<ResourceCommandPanel resource={resource("shipments")} snapshot={pending} />);
    ...
    rerender(<ResourceCommandPanel resource={resource("shipments")} snapshot={handedOff} />);
