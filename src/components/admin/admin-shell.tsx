    ...
    <div className="min-h-svh">
      ...
      <header className="border-b border-border bg-canvas">
        <div className="site-container flex min-h-20 items-center gap-4">
          ...
          <span className="ml-auto hidden lg:inline-flex"><ShellNavLink href="/account">Account</ShellNavLink></span>
          <Sheet>
            <SheetTrigger asChild><Button type="button" variant="outline" size="icon" className="ml-auto size-11 lg:hidden" aria-label="Open administration navigation"><Menu /></Button></SheetTrigger>
      ...
      <div className="site-container grid gap-8 py-10 lg:grid-cols-[15rem_minmax(0,1fr)]">
        <aside className="hidden lg:block"><nav aria-label="Administration" className="sticky top-6 grid gap-1">{resources.map((resource) => <ShellNavLink href={`/admin/${resource.slug}`} key={resource.slug}>{resource.label}</ShellNavLink>)}</nav></aside>
