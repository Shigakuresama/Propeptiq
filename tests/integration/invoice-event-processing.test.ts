...
async function paymentEventCount(): Promise<number> {
...
describe("invoice provider event processing", () => {
  ...
  it.each([
    ["amount paid differs from the durable invoice", "paid-mismatch", { amount_paid: 8_600 }],
    ["amount due differs from the durable invoice", "due-mismatch", { amount_due: 8_600 }],
    ["amount due is nonpositive", "due-zero", { amount_due: 0, amount_paid: 0 }],
    ["amount paid is nonpositive", "paid-zero", { amount_paid: 0 }],
    [
      "the provider says it will charge automatically",
      "automatic-collection",
      { collection_method: "charge_automatically" },
    ],
  ])("rejects invoice.paid when %s", async (_label, suffix, overrides) => {
    const paid = await claim(
      invoiceNormalization("invoice.paid", `evt_inv_${suffix}`, overrides),
      suffix,
    );
    ...
    expect(await paymentEventCount()).toBe(0);
  });

  it.each([
    [
      "the durable invoice amount differs from the order total",
      "durable-due",
      `UPDATE order_invoices SET amount_due_minor = 8600 WHERE order_id = '${ids.order}'`,
    ],
    [
      "the durable invoice amount is nonpositive",
      "durable-zero",
      `UPDATE order_invoices SET amount_due_minor = 0 WHERE order_id = '${ids.order}'`,
    ],
    [
      "the order total differs from the durable invoice",
      "order-total",
      `UPDATE orders SET shipping_minor = 400, total_minor = 8800 WHERE id = '${ids.order}'`,
    ],
  ])("rejects invoice.paid when %s", async (_label, suffix, mutation) => {
    await client.exec(mutation);
    const paid = await claim(
      invoiceNormalization("invoice.paid", `evt_inv_${suffix}`),
      suffix,
    );
  ...
  it.each([
    ["its status is unknown", "unknown-status", { status: "future_invoice_status" }],
    ["its status is restrictive", "restrictive-status", { status: "uncollectible" }],
    [
      "the provider says it will charge automatically",
      "automatic-collection",
      { status: "open", collection_method: "charge_automatically" },
    ],
    ["its amount due differs", "due-mismatch", { status: "open", amount_due: 8_600 }],
    ["its amount due is nonpositive", "due-zero", { status: "open", amount_due: 0 }],
    ["it reports a paid amount", "paid-mismatch", { status: "open", amount_paid: 1 }],
  ])("rejects invoice.payment_failed when %s", async (_label, suffix, overrides) => {
    const paid = await claim(
      invoiceNormalization("invoice.paid", `evt_failed_guard_paid_${suffix}`),
      `failed-guard-paid-${suffix}`,
    );
    await repository().processClaim({ claim: paid, authority: authority(), now });
    expect(await orderState()).toBe("paid_pending_settlement");
    expect(await paymentEventCount()).toBe(1);

    const failed = await claim(
      invoiceNormalization(
        "invoice.payment_failed",
        `evt_failed_guard_${suffix}`,
        overrides,
      ),
      `failed-guard-${suffix}`,
    );
