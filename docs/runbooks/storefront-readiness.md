...
registration and matchers for all four protected route families. Direct Webpack
builds must also run `node scripts/verify-auth-proxy.mjs`, as documented in
`docs/testing.md`. This checks build registration; live requests verify redirects
and session behavior. Keep the proxy beside `src/app`;
