...
Any `pending` or zero-dollar preview record is local/test/preview-only. It may exercise selection and cart behavior outside production, but it cannot create a production Checkout Session. The production UI shows pricing as coming soon rather than claiming a completed purchase. The current canonical production configuration contains no purchasable variants or approved real prices.
