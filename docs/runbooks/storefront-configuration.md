...
Live testing also found a preexisting legacy-cart migration defect. Empty v1 carts incorrectly blocked new canonical items from persisting. The corrective implementation treats empty v1 as ready and preserves nonempty v1 until the existing explicit acknowledgement; it rejects additions while that acknowledgement is required. Its tests and release are tracked separately. Historical candidate-stage notes below describe earlier checkpoints; the production status in this paragraph supersedes their pending-release wording, but does not close missing prices, content, policies, providers, or recurring-commerce decisions.
...
Available footer/support links are Catalog, Cart, Quality Records, Rewards, Partner Program, account Order Tracking, and Research Use Only. The homepage FAQ destination exists at `/#faq`, but its footer entry remains null in the central configuration. Contact or Support, Shipping information, Privacy Policy, Terms and Conditions, Shipping and Returns, Refund Policy, and FDA Disclaimer likewise render no footer link while their routes/copy are missing. There are no corresponding approved Privacy, Terms, Shipping/Returns, Refund, Contact/Support, or FDA routes/copy in the repository. The footer-reference rebuild and remaining requested footer links remain open work.
...
- [ ] Finish the requested footer-reference layout, mobile sticky purchase/cart experience, and remaining responsive/motion acceptance. The existing `/#faq` destination still needs wiring into the footer's null FAQ entry.
