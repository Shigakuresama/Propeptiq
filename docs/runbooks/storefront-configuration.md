...
Catalog cards and canonical product pages use the original blank-vial artwork at [`public/catalog/vial-base-v2.png`](../../public/catalog/vial-base-v2.png). [`src/components/commerce/catalog-product-visual.tsx`](../../src/components/commerce/catalog-product-visual.tsx) adds the current product name, selected/default configuration, and `RESEARCH USE ONLY` label as HTML, so text remains data-driven and the image itself contains no product claim. Replace the base artwork only with an original or licensed asset whose dimensions and visual behavior have been checked across catalog cards, the related carousel, and product pages. The older still-life assets remain only for legacy/browse compatibility and are not used by the canonical card/detail presentation.
...
Related products come only from the explicit ordered relationships in [`src/catalog/storefront-merchandising.ts`](../../src/catalog/storefront-merchandising.ts), never random order. Every current canonical product has two to four configured neighbors. The canonical IDs are joined into each product by [`src/catalog/storefront-catalog-data.ts`](../../src/catalog/storefront-catalog-data.ts), resolved through [`src/catalog/storefront-public.ts`](../../src/catalog/storefront-public.ts), and rendered by [`src/components/commerce/related-products-carousel.tsx`](../../src/components/commerce/related-products-carousel.tsx). Update both sides of a relationship only if that is the approved merchandising decision; do not infer a relationship from names, categories, medical uses, or expected effects.
...
Only approved records can render or enter search. The registry currently contains six neutral Why Choose items, eight operational FAQs, and two neutral product-information records for each of the 56 catalog products. `Why choose PropeptIQ` and FAQ records are projected by [`src/content/storefront-public-content-server.ts`](../../src/content/storefront-public-content-server.ts) and rendered by the server-safe components in [`src/components/site/why-choose-propeptiq.tsx`](../../src/components/site/why-choose-propeptiq.tsx) and [`src/components/site/faq-section.tsx`](../../src/components/site/faq-section.tsx). Product descriptions and exact content IDs are defined in [`src/content/storefront-product-content.ts`](../../src/content/storefront-product-content.ts), attached in canonical product configuration, and projected by [`src/catalog/storefront-public.ts`](../../src/catalog/storefront-public.ts).

Each product currently links to an official PubMed search for its exact catalog name. This is an uncurated literature-discovery link, not an approved study list, endorsement, product claim, or use recommendation. [`src/components/commerce/product-information-sections.tsx`](../../src/components/commerce/product-information-sections.tsx) renders only strict HTTPS PubMed search URLs with one nonblank `term` parameter; other raw source-reference values remain private. Product-specific study summaries, technical facts, storage instructions, or claims must still be added as separately reviewed controlled content and must not be inferred from search results.
...
| Related products, rank, release date, aliases, default variant | [`storefront-merchandising.ts`](../../src/catalog/storefront-merchandising.ts) and [`storefront-catalog-data.ts`](../../src/catalog/storefront-catalog-data.ts) | Owner supplies decisions; developer changes reviewed config | Binding, discovery sort/search, related carousel tests | Two to four explicit relationships exist for all 56 products; rank/release remain unknown |
| Why Choose, FAQ, product info, calculator copy, legal notices | [`storefront-content.ts`](../../src/content/storefront-content.ts) and [`storefront-product-content.ts`](../../src/content/storefront-product-content.ts) | Business/content/legal reviewer approves; developer enters reviewed records | Status/metadata, content policy, homepage/product/footer/search tests | Neutral Why/FAQ/product records are active; calculator/legal records remain gated |
...
- [ ] Approved popularity ranks, release dates, and any additional search aliases. Explicit related-product IDs, canonical presentation artwork, neutral descriptions, and product content IDs are now configured.
- [ ] Any product-specific claims, curated/source-backed study summaries, storage wording, or additional technical information beyond the neutral catalog record and uncurated PubMed discovery link.
