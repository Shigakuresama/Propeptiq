...
The current canonical commerce source in [`src/catalog/storefront-catalog-data.ts`](../../src/catalog/storefront-catalog-data.ts) is deliberately empty. The existing browse manifest therefore does not make any item checkout-ready.
...
The reviewed pre-activation price manifest is [`src/catalog/storefront-catalog-manifest.ts`](../../src/catalog/storefront-catalog-manifest.ts). It is the decision source for deterministic identities, one-vial public labels, and audited candidate base amounts. It currently covers 56 products and 103 scoped rows: 39 owner-approved Amino-equivalent ordinary one-vial list-price candidates and 64 zero-dollar pending rows. Change a reviewed amount, label, source URL, or observation timestamp there only with its exact tests; never use transient competitor promotions as a base price. These 39 amounts are not runtime `active` prices, are not written to `product_prices`, and remain hidden behind `Pricing coming soon` until canonical records, inventory, Stripe mappings, campaign reconciliation, and all other launch approvals are complete. The source `packageForm` strings remain pinned provenance; the public label is the approved one-vial overlay. WINTER30 is applied once by existing server pricing and is never baked into base amounts or stacked with Amino promotions.
...
Pending and unavailable price states sort after active prices. A product card chooses the lowest currently priced available variant, using label and stable ID as deterministic ties; if no priced candidate exists, it can fall back only to the explicit `defaultVariantId`, never the first array element. The card variant selection and displayed price rules are in [`src/catalog/storefront-price-presentation.ts`](../../src/catalog/storefront-price-presentation.ts) and [`src/search/catalog-discovery.ts`](../../src/search/catalog-discovery.ts).
