...
9. The preview token commits to quantity and every customer-visible or continuation-relevant server fact: identity labels, status, availability, base/effective amounts, subtotal, savings, discount, applied promotions, and currency. A quantity or promotion change must change the token even when the effective unit price happens to remain equal.
...
- Invalid JSON or a non-object request returns status 400 with the existing fixed `invalid_request` response and `Cache-Control: no-store`.
