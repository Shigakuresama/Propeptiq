...
2. Preserve the 40 reviewed positive Amino-equivalent **ordinary one-vial base prices** and 63 pending zero-dollar rows, including their source URLs and observation timestamps. No competitor campaign is imported. WINTER30 remains automatic at 30%. The later owner-confirmed policy in `docs/propeptiq-storefront-refactor-contract.md` applies the 3%/6%/30% bottle-volume tiers after WINTER30, per canonical variant line.
...
6. Card selection has one pure authority, reused by discovery price sorting:
   - Honor `defaultVariantId` if it resolves to a non-unavailable variant with a priced presentation and a positive base unit amount under the current pricing context.
   - Otherwise select the lowest displayed effective unit price among those positive-base candidates; keep the existing label-then-ID tie break. An eligible promotion may reduce a positive base amount to zero without turning it into a pending-price placeholder.
   - If no positive-base priced candidate exists, fall back only to the explicit default variant for pending/unavailable/local-zero presentation. Never silently use the first array entry.
