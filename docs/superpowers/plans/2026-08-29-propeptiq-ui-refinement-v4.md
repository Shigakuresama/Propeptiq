...
- Phases A-F were implemented on `ui/refinement-v4`, then transplanted as the exact 63-path UI, regression-test, and isolated-browser-harness change onto the current squash-merged `main` tree in `ui/refinement-v4-release`.
