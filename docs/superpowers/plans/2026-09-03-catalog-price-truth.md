...
**Additional observed generated artifacts:** A broader read-only pre-release dry inspection after the first exclusion pass found449 generated files under `blob-report`, `playwright-report`, and `test-results` still included among1219upload inputs. Exclude these three observed generated-output directories too; preserve their local contents and verify after full browser tests that none are uploaded. Source code, test sources, and required build assets stay included.
