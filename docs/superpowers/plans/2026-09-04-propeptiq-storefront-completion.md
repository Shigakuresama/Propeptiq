...
- [ ] **Cart drawer — unfinished.** Build the requested accessible persistent cart drawer with exact-variant quantities and current totals, reusing the existing cart authority. The current cart page and mobile add controls work; they are not a completed drawer.
...
The cart drawer and the other unchecked tasks above remain open. This correction is deliberately limited to the measured desktop gallery collision.
