...
- [x] **Narrow product-content repair — delivered.** PR #26 contains long product/configuration text and the exact quantity row without clipping or smaller controls. True local 195/240/320px tests pass, and live product-body geometry is verified. This does not close the separate header issue below.
- [ ] **Remaining responsive acceptance — open.** A fresh live 320px check with a 15px classic scrollbar found the header menu button outside the 305px document width. The product body is contained; Task 18B tracks the header repair and scrollbar-aware regression. Do not turn the local full-browser pass into a universal live-layout claim.
...
### Bibliography follow-up — source candidate, not deployed

Task 8B source commit `639bb6fc8ea991f7cc2691eec7e4801c6d86c9d8` adds exact ARA-290, SS-31 and Thymosin Alpha-1 joins: six study PMIDs and two separately linked correction notices. Candidate totals are 20 compounds / 33 studies / 2 notices / zero claims; the previous 17 compound and 27 study records and their review dates are unchanged. The 36 remaining products are not padded with inferred citations. Production through PR #26 remains at 17/27 until this candidate is reviewed, merged, deployed and checked live.
...
- Independent source review found one exact-source regression gap, fixed in test-only commit `a33d22de4dc46f5231aef4665190fa34fac985d1`. Full immutable suffix checks now protect every new compound/study field; focused 83 tests, lint and typecheck passed, and scoped re-review approved with no remaining Important finding. Combined final release gates remain pending. The separate Task 18B compact-header repair follows sequentially on this branch. No price, cart, promotion, checkout or provider behavior changes are included.

### Compact header follow-up — source candidate, not deployed

Task 18B source candidate `f872b0c1dfb1c7a4fd3774b71acf50431501671f` relocates only the existing compact home/wordmark/Cart rules to the existing 22rem breakpoint. RED at a true 305px local viewport reproduced 15px header overflow and menu right 319.97px; GREEN has zero overflow, menu right 289px and all four controls 44x44px. Explicit 320px compact and 375px full-wordmark checks pass. The five new tests, two existing header tests and five unchanged narrow-product tests provide 12 unique passing browser cases; full unit 232 files / 3,528 tests, lint and typecheck passed. Initial test-only destination/typing errors and existing runner/jsdom diagnostics are documented, not represented as pristine output. Independent review, combined full browser/build, deployment and actual live 320/client305 proof remain pending. This paragraph records a new candidate, not a production fix.
...
**Current source/projection status:** Task 8B extends the original 17 mappings/27 studies to 20 exact compound-to-catalog-slug mappings and 33 studies, with two separate correction notices. Original review dates remain September 4; six new study records use September 5. Production through PR #26 is still 17/27 pending this release. The server validates strict identities, design/context compatibility, canonical links, correction relationships and approved-record correspondence before projecting a frozen public bibliography. Public types and neutral wording are isolated in `compound-research-public.ts`. Mechanisms/benefit claims are null and `claims-audit.json` has no claims; studied amounts/routes/outcomes and private approval metadata are not included in the public DTO. This implements a neutral bibliography subset, not all compound research fields, product substantiation, or the missing owner attachments. The candidate leaves 36 products unmapped; notices never inflate study counts. Exact primary-source evidence and publication-status read-back must accompany final Task 8 acceptance.
