...
- [ ] **Mobile purchase/cart controls — unfinished.** The legacy-cart persistence correction is deployed and canonical add/merge/reload is verified live. Complete the sticky add-to-cart/cart experience without hiding permanent search or other controls.
...
### Next bounded mobile purchase release — in progress, not delivered

Add a mobile-only duplicate purchase action after the inline purchase summary has scrolled above the viewport. Reuse the panel's selected canonical variant, validated quantity, price presentation and existing cart action; do not create a second selection or pricing authority. Place it in a shared outside-main dock above permanent centered search, preserve focus and safe-area spacing, and omit the duplicate when JavaScript/Intersection Observer is unavailable. Desktop, pending/invalid/unavailable states, existing search focus and old-cart acknowledgement remain covered. This does not change the 1/2/3/10+ contract, activate checkout, or create subscription products.

The separately observed desktop BPC-157 gallery/search overlap at 1440×1000 remains open. The mobile dock is not permission to add a generalized collision engine or claim all floating-control clearance is complete.
