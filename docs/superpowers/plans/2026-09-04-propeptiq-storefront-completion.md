...
- [ ] **Mobile purchase/cart controls — partially delivered.** PR #21 deploys the mobile purchase dock, preserving the selected variant/quantity/pricing above permanent search. Live add/merge/full-navigation persistence, focus restoration, footer clearance and 375/768/1440px behavior are verified. The cart drawer and separately tracked desktop gallery/search clearance remain unfinished.
...
### Next bounded desktop gallery correction — in progress, not delivered

The separately observed desktop BPC-157 gallery/search overlap at 1440×1000 remains open. The next correction must first reproduce the top-of-page overlap, then use a bounded PDP column-layout adjustment and prove full arrow/tab/panel clearance at desktop widths without changing mobile layout or moving/hiding permanent search. A proposed equal-column central gap is candidate geometry, not accepted browser evidence. Do not add a generalized collision engine or claim all floating-control clearance is complete.
