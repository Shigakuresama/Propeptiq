...
- [ ] **Footer and newsletter — unfinished.** The shared footer FAQ entry is now wired to the existing approved `/#faq` section and browser-tested from home/product pages. Finish the remaining layout/links and privacy content, confirm the existing Resend setup, and activate signup only when it can genuinely subscribe with consent and a working provider. Social `/` placeholders are already authorized.
