...
- Browse-only PDF products retain 53 exact-name cards and all 103 owner-supplied variants without prices or growth actions. Active production database products may show server-authoritative price, package form, promotion, evidence state, and earned points only after a real price and active policy are projected.
