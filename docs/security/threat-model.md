...
Attribution cookies expire after 30 days. Anonymous referral and affiliate lookups rate-limit an HMAC-derived per-caller scope from Vercel's platform-provided client address; the limiter retains neither that raw address nor the raw code, a device fingerprint, or browser return URL. A missing or malformed platform address fails closed outside local development. Only privacy-minimal aggregate or visit facts may be retained for this boundary. Financial, conversion, reward, commission, and payout ledgers follow the applicable order/accounting retention schedule. Redacted administrative audit history is append-only. The accountable owner must define and approve the production retention schedule before activation.
