...
9. In a production identity with every local/demo/test/live capability disabled, no external credential, and `APP_ORIGIN` set to a reserved non-routable HTTPS origin, run the default Turbopack `npm run build` (including its authentication proxy registration check), then `npm run verify:production-artifacts` against that actual deployable output. Preserve the evidence separately, run `npx next build --webpack`, then `node scripts/verify-auth-proxy.mjs`, and scan the Webpack deployable output the same way. Do not contact an external service.
