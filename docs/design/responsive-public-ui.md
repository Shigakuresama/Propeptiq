...
A future `/research-use` route may only redirect to `/research-use-policy`. Account/order routes require authentication. Checkout preserves the anonymous cart through Better Auth sign-in and then collects age 21+, structured purpose, and the current versioned attestation.
